...
ascs_decision_model <- function(x, varnames){
  ...
  for (y in 1:n_years) {
    ...
    if (y == 1) {
      ...
      # because chance that the government intervention has an influence in year 1 is 0
      # is covered by (pre-)assumptions
      if ((y + auxiliary_table$gov_reaction[y]) <= n_years) {
        auxiliary_table$gov_inter_happened_man[y + auxiliary_table$gov_reaction[y]] <- chance_event(auxiliary_table$gov_inter_chance_man[y], 1, 0, 1)
        auxiliary_table$gov_inter_happened_aut[y + auxiliary_table$gov_reaction[y]] <- chance_event(auxiliary_table$gov_inter_chance_aut[y], 1, 0, 1)
      }
      ...
      auxiliary_table$plant_dmg_pot_man[y] <- potential_plant_damage
      # vv(potential_plant_damage, var_CV, n_years, lower_limit = 0.7,upper_limit = 1.3)   
      # vv( potential_plant_damage, 0, n_years)
      auxiliary_table$plant_dmg_pot_aut[y] <- auxiliary_table$plant_dmg_pot_man[y]
      # pesticide quantity
      auxiliary_table$pest_quant_man[y] <- pesticide_quantity     # vv( pesticide_quantity, var_CV, n_years)
      auxiliary_table$pest_quant_aut[y] <- auxiliary_table$pest_quant_man[y] * pest_red_fac
      # water contamination 
      auxiliary_table$rel_water_cont_man[y] <- 1 # rep(1, n_years)
      auxiliary_table$rel_water_cont_aut[y] <- 1 # rep(1, n_years)
      # biodiversity
      auxiliary_table$rel_bio_man[y] <- 1 # rep(1, n_years)
      auxiliary_table$rel_bio_aut[y] <- 1 # rep(1, n_years)
      ...
      ### auxiliary_table$actual_plant_dmg_man[y] <- actual_plant_dmg_man
      ...
      # intervention_delay <- ifelse(round(government_reaction_time) <= y,
      #                              1,
      #                              y - round(government_reaction_time))
       
      if(auxiliary_table$gov_inter_happened_man[y-1] == 0) {
        if ( (y + auxiliary_table$gov_reaction[y] <= n_years) & 
             (auxiliary_table$gov_reaction[y + auxiliary_table$gov_reaction[y]] == 0) ) 
          {
          auxiliary_table$gov_inter_happened_man[y + auxiliary_table$gov_reaction] <- chance_event(auxiliary_table$gov_inter_chance_man[y],
                                                                                                   1, 0, 1)
        }
      }else {auxiliary_table$gov_inter_happened_man[y] <- 1}
      
      if (auxiliary_table$gov_inter_happened_aut[y-1] == 0) {
        if ( (y + auxiliary_table$gov_reaction[y] <= n_years) & 
             (auxiliary_table$gov_reaction[y + auxiliary_table$gov_reaction[y]] == 0) ) 
        {
          auxiliary_table$gov_inter_happened_aut[y + auxiliary_table$gov_reaction] <- chance_event(auxiliary_table$gov_inter_chance_aut[y],
                                                                                                   1, 0, 1)
        }
      }else {auxiliary_table$gov_inter_happened_aut[y] <- 1} 
      ...
      auxiliary_table$plant_dmg_pot_aut[y] <- auxiliary_table$plant_dmg_pot_man[y]
      # pesticide quantity
      auxiliary_table$pest_quant_man[y] <- pesticide_quantity
      auxiliary_table$pest_quant_aut[y] <- auxiliary_table$pest_quant_man[y] * pest_red_fac
      ...
      auxiliary_table$rel_water_cont_man[y] <- auxiliary_table$rel_water_cont_man[y-1] +
        ((1 - auxiliary_table$pest_quant_man[y-1]) * pest_cont_fac)
      auxiliary_table$rel_water_cont_aut[y] <- auxiliary_table$rel_water_cont_aut[y-1] +
        ((1 - auxiliary_table$pest_quant_aut[y-1]) * pest_cont_fac)
      ...
      auxiliary_table$plant_dmg_pot_man[y] <- auxiliary_table$plant_dmg_pot_man[y-1] +
        auxiliary_table$plant_dmg_pot_man[y-1] * (1 -  auxiliary_table$rel_bio_man[y-1])
      auxiliary_table$plant_dmg_pot_aut[y] <- auxiliary_table$plant_dmg_pot_aut[y-1] +
        auxiliary_table$plant_dmg_pot_aut[y-1] * (1 -  auxiliary_table$rel_bio_aut[y-1])
      ...
      actual_plant_dmg_aut <- auxiliary_table$plant_dmg_pot_aut[y] - auxiliary_table$pest_quant_aut[y] / pest_red_fac
      auxiliary_table$actual_plant_dmg_aut[y] <- ifelse(actual_plant_dmg_aut < 0, 0, actual_plant_dmg_aut)
      ### auxiliary_table$actual_plant_dmg_aut[y] <- actual_plant_dmg_aut
    }
