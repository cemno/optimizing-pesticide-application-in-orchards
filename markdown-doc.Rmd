...
ascs_decision_model <- function(x, varnames){
  ...
  for (y in 1:n_years) {
    ...
    if (y == 1) { # first year
      ...
      ## debugging
      # print(c("man", y, auxiliary_table$gov_inter_chance_man[y]))
      # print(c("aut", y, auxiliary_table$gov_inter_chance_aut[y]))
      
      
      # chance_event function 
      ## only applies ifthe value for the intervention delay + the current year is still inside the n_years boarder &
      ### there was no intervention in the previous years
      ### there was no intervention in the relevant year already
      
      print(c("before if", y, auxiliary_table$gov_reaction[y] ))
      
      if (y + auxiliary_table$gov_reaction[y] <= n_years)
      {
        
        print( c("gov_inter_happ y-1", auxiliary_table$gov_inter_happened_man[y-1]) )
        print( c("chance", auxiliary_table$gov_inter_chance_man[y]))
        
        if (auxiliary_table$gov_inter_happened_man[y-1] == 0)
        {  
          print( c("gov_react[y+gov_react]", auxiliary_table$gov_reaction[y + auxiliary_table$gov_reaction[y]], "++++++++++++++++++++") )
          if (auxiliary_table$gov_reaction[y + auxiliary_table$gov_reaction[y]] == 0)
          {
            
            print(c("rbinom", rbinom(1, 1, auxiliary_table$gov_inter_chance_man[y]), "------------------------------------"))
            
            auxiliary_table$gov_inter_happened_man[y + auxiliary_table$gov_reaction[y]] <- rbinom(1, 1, auxiliary_table$gov_inter_chance_man[y])
                                                            ##### as.numeric(chance_event(auxiliary_table$gov_inter_chance_man[y], 1, 0, 1))
          }
        ...
        if (auxiliary_table$gov_inter_happened_aut[y-1] == 0) 
        {
          if (auxiliary_table$gov_reaction[y + auxiliary_table$gov_reaction[y]] == 0)
          {
            auxiliary_table$gov_inter_happened_aut[y + auxiliary_table$gov_reaction[y]] <- rbinom(1, 1, auxiliary_table$gov_inter_chance_aut[y])
                                                            ###### chance_event(auxiliary_table$gov_inter_chance_aut[y], 1, 0, 1)
          }
