...
ascs_decision_model <- function(x, varnames){
  ...
  for (y in 1:n_years) {
    ...
    # if (y == 1) {
    #
    #   test[y,rel_water_cont_man] <- 1
    #   test[y,rel_water_cont_aut] <- 1
    #   test[y,rel_bio_man] <- 1
    #   test[y,rel_bio_aut] <- 1
    #   
    #   test[y,plant_dmg_pot_man] <- vv( potential_plant_damage, var_CV, n_years)
    #   test[y,plant_dmg_pot_aut] <- vv( potential_plant_damage, var_CV, n_years)
    #   
    #   test[y,actual_plant_dmg_man] <- plant_damage_pot - pest_quant_man
    #   test[y,actual_plant_dmg_aut] <- plant_damage_pot - pest_quant_aut / pest_red_fact
    #   
    # } else {
      if (y > 1) {
      
      test[y,rel_water_cont_man] <- test[(y-1),rel_water_cont_man] + ((1 - test[(y-1),pest_quant_man])*0.1)
      test[y,rel_water_cont_aut] <- test[(y-1),rel_water_cont_aut] + ((1 - test[(y-1),pest_quant_aut])*0.1)
      test[y,rel_bio_man] <- test[(y-1),rel_biodiversity_man] + ((1 - test[(y-1),pest_quant_man])*0.1)
      test[y,rel_bio_aut] <- test[(y-1),rel_biodiversity_aut] + ((1 - test[(y-1),pest_quant_aut])*0.1)
      
      test[y,plant_dmg_pot_man] <- test[(y-1),plant_dmg_pot_man] + (rel_water_cont_man + rel_bio_man)
      test[y,plant_dmg_pot_aut] <- test[(y-1),plant_dmg_pot_aut] + (rel_water_cont_aut + rel_bio_aut)
      
      actual_plant_dmg_man <- test[(y-1),plant_damage_pot_man] - test[(y-1),pest_quant_man]
      test[y,actual_plant_dmg_man] <- ifelse(actual_plant_dmg_man<0,0,actual_plant_dmg_man)
      actual_plant_dmg_aut <- test[(y-1),plant_damage_pot_aut] - test[(y-1),pest_quant_aut] / pest_red_fact
      test[y,actual_plant_dmg_aut] <- ifelse(actual_plant_dmg_aut<0,0,actual_plant_dmg_aut)
      }
