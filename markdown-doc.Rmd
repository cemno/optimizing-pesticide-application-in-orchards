...
input_table <- read_csv("data/final/inputs.csv")
...
ascs_decision_model <- function(x, varnames){
  ...
  # test <- data.frame(year = integer(),
  #                    pest_quant_man = double(),
  #                    pest_quant_aut = double(),
  #                    rel_water_cont_man = double(),
  #                    rel_water_cont_aut = double(),
  #                    rel_bio_man = double(),
  #                    rel_bio_aut = double(),
  #                    potential_plant_dmg_man = double(),
  #                    potential_plant_dmg_aut = double(),
  #                    actual_plant_dmg_man = double(),
  #                    actual_plant_dmg_aut = double()
  #                    )
  test <- data.frame(year = rep(1:10),
                     pest_quant_man = rep(0,10),
                     pest_quant_aut = rep(0,10),
                     rel_water_cont_man = rep(0,10),
                     rel_water_cont_aut = rep(0,10),
                     rel_bio_man = rep(0,10),
                     rel_bio_aut = rep(0,10),
                     potential_plant_dmg_man = rep(0,10),
                     potential_plant_dmg_aut = rep(0,10),
                     actual_plant_dmg_man = rep(0,10),
                     actual_plant_dmg_aut = rep(0,10))
  rel_water_cont_man = 5000
  test[year == 1,rel_water_cont_man] <- rel_water_cont_man
  test[2,rel_water_cont_man] <- test[1,rel_water_cont_man] + ((1 - test[1,rel_water_cont_man])*0.1)
    
  test$pest_quant_man <- vv( pesticide_quantity, var_CV, n_years)
  test$pest_quant_aut <- vv( pesticide_quantity, var_CV, n_years) * pesticide_reduction_auto 
  
  test$rel_water_cont_man <- rep(1, n_years)
  test$rel_water_cont_aut <- rep(1, n_years)
  test$rel_bio_man <- rep(1, n_years)
  test$rel_bio_aut <- rep(1, n_years)
  
  test$plant_dmg_pot_man <- vv( potential_plant_damage, var_CV, n_years)
  test$plant_dmg_pot_aut <- vv( potential_plant_damage, var_CV, n_years)

#  test$actual_plant_dmg_man <- plant_damage_pot - pest_quant_man           ############# noch fertig machen [y-1] berücksichtigen
#  test$actual_plant_dmg_aut <- plant_damage_pot - pest_quant_aut / pest_red_fact
  ...
  for (y in 1:n_years) {
    ...
    test[y, "year"] <- y
      ...
      test$rel_water_cont_man[y] <- test$rel_water_cont_man[y-1] + ((1 - test$pest_quant_man[y-1])*0.1)
      test$rel_water_cont_aut[y] <- test$rel_water_cont_aut[y-1] + ((1 - test$pest_quant_aut[y-1])*0.1)
      test$rel_bio_man[y] <- test$rel_biodiversity_man[y-1] + ((1 - test$pest_quant_man[y-1])*0.1)
      test$rel_bio_aut[y] <- test$rel_biodiversity_aut[y-1] + ((1 - test$pest_quant_aut[y-1])*0.1)
      
      test$plant_dmg_pot_man[y] <- test$plant_dmg_pot_man[y-1] + (rel_water_cont_man + rel_bio_man)
      test$plant_dmg_pot_aut[y] <- test$plant_dmg_pot_aut[y-1] + (rel_water_cont_aut + rel_bio_aut)
      
      actual_plant_dmg_man <- test$plant_damage_pot_man[y-1] - test$pest_quant_man[y-1]
      test$actual_plant_dmg_man[y] <- ifelse(actual_plant_dmg_man<0,0,actual_plant_dmg_man)
      actual_plant_dmg_aut <- test$plant_damage_pot_aut[y-1] - test$pest_quant_aut[y-1] / pest_red_fact
      test$actual_plant_dmg_aut[y] <- ifelse(actual_plant_dmg_aut<0,0,actual_plant_dmg_aut)
      }
  ...
  # we assume the water contamination decreases every year by 5% without any influence
  ...
  # assuming that the amount that we spray more then the government guideline influences the water contamination by 10%
  rel_water_cont_man <- rel_water_cont_dec + ((1 - pest_quant_man)*0.1)
  rel_water_cont_aut <- rel_water_cont_dec + ((1 - pest_quant_aut)*0.1)
  
  # we would have a natural increase in biodiversity if we did not spray
  rel_biodiversity_inc <- vv(rel_biodiversity,
                             5, #var_CV,
                             n_years
                             # relative_trend = 15
                             )
  # assuming that the amount that we spray influences the biodiversity by 10%
  rel_bio_man <- rel_biodiversity_inc + ((1 - pest_quant_aut)*0.1)
  rel_bio_aut <- rel_biodiversity_inc + ((1 - pest_quant_man)*0.1)


  # # calculating the theoretic pest quantity 
  # plant_dmg_boundary <- inputs[inputs_table$variable=="potential_plant_damage",c("lower","upper")]
  # perfect_pest_quantity <- (plant_dmg_boundary$upper - plant_dmg_boundary$lower)
  # 
  # theoretic_pest_quantity <- plant_damage_pot * perfect_pest_quantity 

  # potential plant damage
  plant_damage_pot <- vv( potential_plant_damage, var_CV, n_years ) # 0.7 - 1.7
  
  ####### plant damage pot changes depending on biodiversity
  
  # actual plant damage after applying pesticides 
  actual_plant_dmg_man <- plant_damage_pot - pest_quant_man
  actual_plant_dmg_man <- ifelse(actual_plant_dmg_man<0,0,actual_plant_dmg_man)
  actual_plant_dmg_aut <- plant_damage_pot - pest_quant_aut / pest_red_fact
  actual_plant_dmg_aut <- ifelse(actual_plant_dmg_aut<0,0,actual_plant_dmg_aut)
  
  # yields prizes are varying depending on the years and the plant damage
  yield_t_ha <- vv(yield, var_CV, n_years)
  yield_t_ha_man <- yield_t_ha * (1 - actual_plant_dmg_man)
  yield_t_ha_aut <- yield_t_ha * (1 - actual_plant_dmg_aut)
  ...
  pest_cost_ha_man <- pest_cost_ha_const * pest_quant_man
  pest_cost_ha_aut <- pest_cost_ha_const * pest_quant_aut
...
input_table <- as.estimate(input_table, correlation_matrix = correlation_matrix)
